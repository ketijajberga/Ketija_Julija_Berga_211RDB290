"""
VPN veiktspējas mērījumu statistikas aprēķins — normālie tīkla apstākļi
Ievaddati:  Normali_Neapstradatie_merijumi.xlsx
Izvaddati:  Normali_Statistika_rezultati.xlsx
"""
...
INPUT_FILE  = "Normali_Neapstradatie_merijumi.xlsx"
OUTPUT_FILE = "Normali_Statistika_rezultati.xlsx"
...
METRICS = {
    ...
    "Savienojuma laiks (s)": "Savien. laiks (s)",
}

STAT_LABELS = ["Vid.", "Med.", "St.nov.", "Min.", "Maks.", "P95"]

SCENARIOS = [
    ("LAN-LAN",             ["VM1→VM2", "VM2→VM1"]),
    ("Attālinātā piekļuve", ["VM3→VM1"]),
    ("Mākoņintegrācija",    ["VM1→VM4"]),
]

# ============================================================
# 1. DATU IELĀDE
# Katras lapas pirmā rinda ir virsraksts, dati sākas no otrās rindas (header=1)
# Pēc apvienošanas izfiltrē tukšās un galvenes rindas
# ============================================================

print("Lasu datus no:", INPUT_FILE)
all_sheets = pd.read_excel(INPUT_FILE, sheet_name=None, header=1)

frames = [df for df in all_sheets.values()]
raw = pd.concat(frames, ignore_index=True)

# Izfiltrē rindas, kur Risinājums nav aizpildīts vai ir atkārtota galvene
raw = raw[raw["Risinājums"].notna() & (raw["Risinājums"] != "Risinājums")]

print(f"  Kopā rindas: {len(raw)}")

# ============================================================
# 2. STATISTIKAS APRĒĶINS
# Katrai grupai (risinājums + scenārijs + virziens) aprēķina
# 6 statistikas rādītājus katrai metrikai
# ============================================================
...
for scenario_label, directions in SCENARIOS:
    ...
    ws["A1"] = f"Normālie tīkla apstākļi — {scenario_label} scenārijs"
    ws["A1"].font = Font(name="Arial", bold=True, size=12, color="1F4E79")
    ws["A1"].alignment = CENTER
    ws.row_dimensions[1].height = 22

    # 2. rinda — risinājumu virsraksti
    c = ws.cell(row=2, column=1, value="Virziens")
    ...
    # Datu rindas — viena rinda uz virziens
    for i, direction in enumerate(directions):
        row = 5 + i
        ws.row_dimensions[row].height = 16
        fill = ALT if i % 2 == 0 else PatternFill()

        c = ws.cell(row=row, column=1, value=direction)
        c.font = B_FONT; c.alignment = CENTER; c.border = BRD
        if i % 2 == 0:
            c.fill = fill

        col = 2
        for sol in SOLUTIONS:
            # Filtrē pēc risinājuma, scenārija un virziena
            mask = (
                (raw["Risinājums"] == sol) &
                (raw["Scenārijs"]  == scenario_label) &
                (raw["Virziens"]   == direction)
            )
            df_filt = raw[mask]

            for col_name in METRICS.keys():
                vals = calc_stats(df_filt[col_name]) if col_name in df_filt.columns else [None] * 6
                for val in vals:
